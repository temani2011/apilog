...
use Context;
use Module;
use EO\ApiLog\Grid\Definition\Factory\ApiLogProductsPackagesGridDefinitionFactory;
use EO\ApiLog\Grid\Filters\ApiLogProductsPackagesFilters;
...
class ApiLogProductsPackagesController extends FrameworkBundleAdminController
{
    public $tabName = 'Изменение в упаковках';

    /**
     * List api_log
     *
     * @param ApiLogProductsPackagesFilters $filters
     *
     * @return Response
     */
    public function indexAction(ApiLogProductsPackagesFilters $filters): Response
    {
        $gridFactory = $this->get('api_log.grid.factory.products_packages');
        $grid = $gridFactory->getGrid($filters);

        return $this->render('@Modules/eo_api_log/views/templates/admin/api_log/index.html.twig', [
            'enableSidebar'          => true,
            'layoutTitle'            => 'Изменение в упаковках',
    ...
    public function searchAction(Request $request): RedirectResponse
    {
        ...
        return $responseBuilder->buildSearchResponse(
            $this->get('api_log.grid.definition.factory.products_packages'),
            $request,
            ApiLogProductsPackagesGridDefinitionFactory::GRID_ID,
            'admin_api_log_products_packages_list'
        );
    }
    ...
    public function returnAction(int $id)
    {
        ...
        try {
            $product = $repository->restoreProduct($id);
        } catch (\Throwable $th) {
            $errors[] = [
        ...
        return $this->redirectToRoute('admin_api_log_products_packages_list');
    }
     ...
     * @return RedirectResponse
     */
    public function bulkReturnAction(ApiLogProductsPackagesFilters $filters, Request $request)
    {
        ...
        $results = $repository->restoreProduct($ids);
     ...
     * @return array
     */
    private function getBulkFromRequest(ApiLogProductsPackagesFilters $filters, Request $request)
    {
        if ($request->query->get('select-all') === 'true') {
            $gridFactory = $this->get('api_log.grid.data.factory.products_packages');
