...
/**
 * Class ApiLogProductsQueryBuilder.
 */
final class ApiLogProductsQueryBuilder extends AbstractDoctrineQueryBuilder
{
    /**
    ...
    private function getQueryBuilder(array $filterValues): QueryBuilder
    {
        ...
        $qb->from($this->dbPrefix . 'api_log_products');
        ...
        foreach ($filterValues as $filterName => $filter) {
            ...
            if ('id_product' === $filterName) {
                $qb->andWhere('id_product = :id_product');
            ...
            if ('manufacturer' === $filterName) {
                $qb->andWhere('manufacturer LIKE :manufacturer');
                $qb->setParameter('manufacturer', '%' . $filter . '%');
                continue;
            }
            ...
            if ('reference' === $filterName) {
                $qb->andWhere('reference LIKE :reference');
            ...
            if ('multiplicity' === $filterName) {
                $qb->andWhere('multiplicity = :multiplicity');
                $qb->setParameter('multiplicity', $filter);
                continue;
            }

            if ('min_pack' === $filterName) {
                $qb->andWhere('min_pack = :min_pack');
                $qb->setParameter('min_pack', $filter);
                continue;
            }
