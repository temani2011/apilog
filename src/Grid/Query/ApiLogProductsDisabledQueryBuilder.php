...
/**
 * Class ApiLogProductsDisabledQueryBuilder.
 */
final class ApiLogProductsDisabledQueryBuilder extends AbstractDoctrineQueryBuilder
{
    /**
    ...
    private function getQueryBuilder(array $filterValues): QueryBuilder
    {
        ...
        $qb->from($this->dbPrefix . 'api_log_products_disabled', 'alpp');
        ...
        foreach ($filterValues as $filterName => $filter) {
            ...
            if ('status' === $filterName) {
                $qb->andWhere('alpp.status = :status');
